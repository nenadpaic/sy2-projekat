...
class Pictures implements \Serializable
{
     ...
     * @Assert\Regex(pattern="/^[a-zA-Z0-9_' -]+$/i",htmlPattern = "^[a-zA-Z0-9_]+$", match=true, message="Title can contain only alphanumeric characters")
     * @Assert\NotBlank()
     * @Assert\Length(min=4, max=20, minMessage="Title can not be less than {{ limit }} long", maxMessage="Title can not be longer than {{ limit }} characters long")
     */
    private $name;
